...
export class DropDataItemHandler extends EventHandler {
  ...
  static makeDropable(elem: Element, onDrop?: IDropHandler, options: IDropDataItemHandlerOptions = {}) {
    return new DropDataItemHandler(elem, onDrop, options);
  }

  static makeDraggable<T>(sel: d3.Selection<T>, dataGetter: (d: T) => IDataType = (d: any) => d) {
    sel
      .attr('draggable', true)
...
export class DataBrowser extends EventHandler {
  ...
  private buildTree(parent: Element) {
    ...
    function buildLevel($level: d3.Selection<INode>) {
      ...
      const $label = $childsEnter.append('span')
        ...
        .call(DropDataItemHandler.makeDraggable, (d: INode) => d.data);
  ...
  static createDataBrowser(parent: HTMLElement, options: IDataBrowserOptions = {}) {
